...
function MatchUp(p0,p1,prev1,prev2) {
	...
	this.draw = function(ctx,x,y,w,h,direction,sx,sy,ex,ey) {
		...
		if ("left" == direction) {
			this.outX = sx+w;
			ctx.fillText(p0,sx+5,sy-10,w-10);
			ctx.moveTo(sx,sy);
			ctx.lineTo(this.outX,sy);
			ctx.stroke();
			ctx.fillText(p1,sx+5,ey-10,w-10);
...
//			ctx.lineTo(this.outX,sy+h);
			...
			this.outX = sx-w;
			ctx.fillText(p0,this.outX+5,sy-10,w-10);
			ctx.moveTo(this.outX,sy);
			ctx.lineTo(sx,sy);
			ctx.stroke();
			ctx.fillText(p1,this.outX+5,ey-10,w-10);
			ctx.moveTo(this.outX,ey);
			ctx.lineTo(sx,ey);
			ctx.stroke();
			ctx.moveTo(this.outX,sy);
